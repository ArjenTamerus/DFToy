...
/* ----------------------------------------------------
  ...
  for (iter=1;iter<=max_iter;iter++) {
	...
	for (i=0;i<num_pw;i++) {
	  ...
	  cg_beta += search_direction[offset+2*i]*gradient[offset+2*i];
	  cg_beta += search_direction[offset+2*i+1]*gradient[offset+2*i+1];
	}
      }
	...
	for (nb=0;nb<num_states;nb++) {
	  ...
	  for (i=0;i<num_pw;i++) {
	    // The complex dot-product is conjg(trial_wvfn)*gradient
	    // we just want the real part here
	    search_direction[offset+2*i]   += cg_gamma*prev_search_direction[offset+2*i];
	    search_direction[offset+2*i+1] += cg_gamma*prev_search_direction[offset+2*i+1];
	  }
	}
...
/* |-------------------------------------------------|
 ...
 /* |---------------------------------------------------------------------|
    ...
    for (np=0;np<num_pw;np++) {
      ...
      kinetic_eigenvalue += H_kinetic[np]*pow(trial_wvfn[offset+2*np],2);
      kinetic_eigenvalue += H_kinetic[np]*pow(trial_wvfn[offset+2*np+1],2);
    }
    ...
    for (np=0;np<num_pw;np++) {
      ...
      temp = 8.0 + x*(4.0 + x*(2.0 + 1.0*x));
      search_direction[offset+2*np]   *= temp/(temp+pow(x,4));
      search_direction[offset+2*np+1] *= temp/(temp+pow(x,4));
...
/* |-------------------------------------------------|
   ...
   lapack_real_work = (double *)malloc((3*num_states-2)*sizeof(double));

   lapack_lwork = 2*num_states-1;
   lapack_cmplx_work = (double *)malloc(2*lapack_lwork*sizeof(double));
  
   for (i=0;i<3*num_states-2;i++) { lapack_real_work[i] = 0.0; }
   for (i=0;i<2*lapack_lwork;i++) { lapack_cmplx_work[i] = 0.0; }

   // Diagonalise to get eigenvectors and eigenvalues
   jobz = 'V';
   uplo ='U';
   zheev_(&jobz,&uplo,&num_states,rotation,&num_states,eigenvalues,lapack_cmplx_work, 
	  &lapack_lwork,lapack_real_work,&status);
   if ( status != 0 ) { 
